...
QString DbManager::generatePreviewHtml(const ApprovalRecord& record,
                                       const QString& templatePath,
                                       QString& errorMessage) {
    QString python = findPython();
    if (python.isEmpty()) {
    ...
    QString appDir = findAppDir();
    QString script = appDir + "/generate_preview.py";
    if (!QFileInfo::exists(script)) {
        script = QDir::cleanPath(qApp->applicationDirPath() + "/../../server/generate_preview.py");
    }
    if (!QFileInfo::exists(script)) {
        errorMessage = "找不到 generate_preview.py 脚本: " + script;
        return "";
    }
    QString realTemplate = templatePath;
    if (!QFileInfo::exists(realTemplate)) {
    ...
    QTemporaryDir tempDir(QDir::temp().filePath("cns_preview_XXXXXX"));
    if (!tempDir.isValid()) {
        errorMessage = "无法创建临时目录";
        return "";
    }
    ...
    QStringList args;
    args << script << realTemplate << recordJsonPath << appDir;
    process.start(python, args);

    if (!process.waitForStarted(15000)) {
        errorMessage = "启动预览脚本失败: " + process.errorString();
        return "";
    }
    ...
    QString stderr_str = QString::fromUtf8(process.readAllStandardError());
    if (process.exitCode() != 0) {
        errorMessage = QString("预览生成失败 exit=%1 stderr=%2").arg(process.exitCode()).arg(stderr_str);
        return "";
    }
    return stdout_str;
}
